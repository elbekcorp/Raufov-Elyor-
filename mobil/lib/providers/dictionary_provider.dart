...
class DictionaryProvider with ChangeNotifier {
  static const String baseUrl = "http://10.0.2.2:8000"; // Android Emulator için backend URL
  List<Word> _words = [];
  List<Word> _userWords = [];
  List<Word> _filteredWords = [];
  bool _isLoading = true;

  List<Word> get words => _filteredWords.isEmpty && !_isLoading ? (_userWords + _words) : _filteredWords;
  ...
  Future<void> loadWords() async {
    try {
      ...
      try {
        final response = await http.get(Uri.parse("$baseUrl/words?limit=1000")).timeout(const Duration(seconds: 3));
        ...
        final String localResponse = await rootBundle.loadString('assets/data/words.json');
  ...
  Future<void> addWord(String uz, String en, String ru) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final String userWordsJson = json.encode(_userWords.map((e) => {
      'uz': e.uz,
      'en': e.en,
      'ru': e.ru,
    }).toList());
  ...
  void search(String query) async {
    ...
    EasyDebounce.debounce(
      ...
      () async {
        ...
        try {
          final response = await http.get(Uri.parse("$baseUrl/search?q=$query")).timeout(const Duration(seconds: 2));
          if (response.statusCode == 200) {
            final data = json.decode(response.body) as List;
          ...
          debugPrint("Backend qidiruvda xato: $e. Lokal qidiruv ishlatilmoqda.");
        }
